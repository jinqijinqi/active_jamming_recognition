%2020.11.18
%产生ISRJ+aiming_jam干扰作为分类数据，干噪比30-60dB之间随机；
close all;clear;clc
j=sqrt(-1);
data_num=500;   %干扰样本数
samp_num=2000;%距离窗点数
...
for m=1:data_num
    ...
    range_tar=1+round(rand(1,1)*1200);
    ...
    range=150+round(rand(1,1)*180);%假目标滞后1~10us，即20-200个点
    sp(1+range_tar+range:length(lfm)+range_tar+range)=sp(1+range_tar+range:length(lfm)+range_tar+range)+Aj/2*lfm;
...
% save('F:\deep_learning_for_active_jamming_2020.11.16\jamming_data\ISRJ_with_distance_10\echo.mat' ,'echo')
% save('F:\deep_learning_for_active_jamming_2020.11.16\jamming_data\ISRJ_with_distance_10\echo_stft.mat' ,'echo_stft')
% save('F:\deep_learning_for_active_jamming_2020.11.16\jamming_data\ISRJ_with_distance_10\label.mat' ,'label')
