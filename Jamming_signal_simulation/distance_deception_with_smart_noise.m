%2020.11.18
%产生距离欺骗+灵巧噪声（噪声*ISRJ）干扰作为分类数据，干噪比30-60dB之间随机
close all;clear;clc
j=sqrt(-1);
data_num=500;   %干扰样本数
samp_num=2000;%距离窗点数
...
for m=1:data_num
    ...
    sp(1+range_tar+range:length(lfm)+range_tar+range)=sp(1+range_tar+range:length(lfm)+range_tar+range)+Aj/2*lfm;%噪声+目标回波+干扰 1
...
% save('F:\deep_learning_for_active_jamming_2020.11.16\jamming_data\distance_deception_with_smart_noise_8\echo.mat' ,'echo')
% save('F:\deep_learning_for_active_jamming_2020.11.16\jamming_data\distance_deception_with_smart_noise_8\echo_stft.mat' ,'echo_stft')
% save('F:\deep_learning_for_active_jamming_2020.11.16\jamming_data\distance_deception_with_smart_noise_8\label.mat' ,'label')
